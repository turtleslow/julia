...
@testset "replace! & replace" begin
    ...
    # test nothing
    a = Any[1, 2, nothing, 4]
    @test replace(x -> x === nothing ? 0 : nothing, a) == [nothing, nothing, 0, nothing]
    @test replace(x -> x === nothing ? 0 : x, a) == [1, 2, 0, 4]
    @test replace!(x -> x !== nothing ? nothing : Some(x), a) == [nothing, nothing, Some(nothing), nothing]
    @test replace(iseven, Any[1, 2, 3, 4], nothing) == [1, nothing, 3, nothing]
    @test replace(Any[1, 2, 3, 4], 1=>Some(nothing), 3=>nothing) == [Some(nothing), 2, nothing, 4]
    s = Set([1, 2, nothing, 4])
    @test replace(x -> x === nothing ? 0 : nothing, s) == Set([0, nothing])
    @test replace(x -> x === nothing ? 0 : x, s) == Set([1, 2, 0, 4])
    @test replace(x -> x !== nothing ? nothing : x, s) == Set([nothing])
    @test replace(iseven, Set(Any[1, 2, 3, 4]), nothing) == Set([1, nothing, 3, nothing])
    @test replace(Set(Any[1, 2, 3, 4]), 1=>Some(nothing), 3=>Some(nothing)) == Set([Some(nothing), 2, 4])
