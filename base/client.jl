...
function exec_options(opts)
    ...
    if repl
        ...
        if interactiveinput
            ...
            run_main_repl(quiet, banner, history_file, color_set)
        else
            banner = (opts.banner == 1) # --banner=yes
            banner && banner()
            include_stream(Main, STDIN)
        end
    end
    nothing
end
...
function run_main_repl(quiet::Bool, banner::Bool, history_file::Bool, color_set::Bool)
    if isassigned(REPL_MODULE_REF)
        ...
        quiet || @warn "REPL provider not available: using basic fallback"
        banner && Base.banner()
        while isopen(STDIN)
            print("julia> ")
            flush(STDOUT)
...
# reading from a file or stream, behaving like include
function include_stream(mod::Module, in::IO)
    if isa(in, File) || isa(in, IOStream)
        # for files, we can slurp in the whole thing at once
        eval(mod, parse_input_line(read(in, String)))
    else
        # for unknown streams, operate by line
        while !eof(in)
            eval(mod, parse_input_line(in))
        end
    end
    nothing
end
